...
/*
We can solve this problem by using the closest pair algorithm to find out the two currently closest ships and have them change their courses.
The Brute force solution is O(n^2), compute the distance between each pair and return the smallest. We can calculate the smallest distance in O(nLogn) time using Divide and Conquer strategy. 
 formula for distance between two points p and q.
